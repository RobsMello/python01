...
async def get_usuarios(db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        usuarios: List[UsuariosModel] = result.scalars().unique().all()
...
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        usuario: UsuariosSchemaArtigos = result.scalars().unique().all()
...
async def put_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario_up: UsuarioSchemaBase = result.scalars().unique().all()
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        usuario_del: UsuariosSchemaArtigos = result.scalars().unique().all()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    return JSONResponse(content={"acess_token": criar_token_acesso(sub=usuario.id), "token_type": "bearer"},
